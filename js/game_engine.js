...
const assets = new AssetLoader(); // グローバルオブジェクト
...
class InputReceiver{
    constructor(){
        ...
        addEventListener('keydown', (ked) => this._keyMap.set(ked.key, true));
        addEventListener('keyup', (keu) => this._keyMap.set(keu.key, false));
    }
...
class Scene extends EventDispatcher{
    constructor(name, bgColor, renderingTarget){
        super();
...
class Game{
    ...
    _loop(timeStamp){
        const deltaSec = (timeStamp - this._prevTimeStamp)/1000;
        const accuracy = 0.9; //あまり厳密にするとフレームが飛んだりすることがあるらしい
        const frameTime = 1/this.maxFps*accuracy; //精度を落としている
        if(deltaSec <= frameTime){
            requestAnimationFrame(this._loop.bind(this));
            return;
        }

        this._prevTimeStamp = timeStamp;
        this.currentFps = 1/deltaSec;
